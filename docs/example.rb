...
module Chartable
  ...
  class_methods do
    def chart_data(site_id, sensor_id, between, select_data)
      ...
      query = <<~SQL
              ...
              (
                  ...
                  WHERE (at >= toDateTime(#{between.first.to_i})) AND (at <= toDateTime(#{between.last.to_i})) AND (site_id = #{site_id}) AND (sensor_id = #{sensor_id})
                  GROUP BY at
        )
              GROUP BY at
              ORDER BY at ASC
